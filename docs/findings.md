...
3. **Candidate edge law (post-hoc, not yet a claim)**: all eight cells in the prefix/full family fit ρ ≈ 0.033·K·η within ~±12% — i.e. the parameter-space edge sits at K·η ≈ 30 for this T and family. Flagged for pre-registration in the next transport experiment (vary T, third η, other families) before any use; the block-shuffle point at matched K sits 30% off the law, so the prefactor carries inversion-set geometry.
