...
- **Parallel tracks (non-blocking, both outward-facing steps need user sign-off)**:
  - **(b)**: draft request to AllenAI for the signal-and-noise 1B seed-model weights; if ever obtained → continuity audit first, then a fresh pre-registration against its unseen curves. Watch their HF org. **Status 2026-07-21: drafted at docs/allenai_email_draft.md (recipient `davidh@allenai.org`, the corresponding author); availability re-checked the same day — the paper claims release but only the eval dataset and code are public, so the ask stands. NOT SENT — no mail capability in-harness; the user sends.**
